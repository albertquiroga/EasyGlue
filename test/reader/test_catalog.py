...
class TestCatalogRead(unittest.TestCase):
    ...
    def test_validate_qualified_name(self):
        from easyglue.reader._catalog import _validate_qualified_name
        database, table = _validate_qualified_name(self, "default.legislators")
        self.assertEqual("default", database)
        self.assertEqual("legislators", table)

    def test_validate_qualified_name_error(self):
        from easyglue.reader._catalog import _validate_qualified_name
        self.assertRaises(ValueError, _validate_qualified_name, self, "legislators")
